...
/*
**  Data malloc :  cols+32 ( bitmap_pad=32 ),    *4 = *32 / 8bit
*/


void	*mlx_int_new_xshm_image(t_xvar *xvar,int cols,int rows,int format)
{
  t_img	*img;
  int	(*save_handler)();
  ...
  img->image = XShmCreateImage(xvar->display,xvar->visual,xvar->depth,
			       format,img->data,&(img->shm),cols,rows);
  if (!img->image)
    {
      free(img);
      return ((void *)0);
  ...
  img->format = format;
  img->shm.shmid = shmget(IPC_PRIVATE,(cols+32)*rows*4,IPC_CREAT|0777);
  if (img->shm.shmid==-1)
    {
...
void	*mlx_int_new_image(t_xvar *xvar,int cols, int rows,int format)
{
  t_img	*img;

  if (!(img = malloc(sizeof(*img))))
    return ((void *)0);
  if (!(img->data = malloc((cols+32)*rows*4)))
  {
    free(img);
    return ((void *)0);
  }
  bzero(img->data,(cols+32)*rows*4);
  img->image = XCreateImage(xvar->display,xvar->visual,xvar->depth,format,0,
			    img->data,cols,rows,32,0);
  if (!img->image)
    {
      free(img->data);
      free(img);
  ...
  img->cols = cols;
  img->rows = rows;
  img->pix = XCreatePixmap(xvar->display,xvar->root,cols,rows,xvar->depth);
  img->format = format;
...
void	*mlx_new_image(t_xvar *xvar,int cols, int rows)
{
  t_img	*img;

  if (xvar->use_xshm)
    if (img = mlx_int_new_xshm_image(xvar,cols,rows,ZPixmap))
      return (img);
  return (mlx_int_new_image(xvar,cols,rows,ZPixmap));
}

void	*mlx_new_image2(t_xvar *xvar,int cols, int rows)
{
  t_img	*img;

  if (xvar->use_xshm)
    if (img = mlx_int_new_xshm_image(xvar,cols,rows,XYPixmap))
      return (img);
  return (mlx_int_new_image(xvar,cols,rows,XYPixmap));
}
