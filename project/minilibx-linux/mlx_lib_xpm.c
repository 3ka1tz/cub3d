...
void		*mlx_int_xpm_f_image(t_xvar *xvar,int *cols,int *rows,
				     int (*xpm_func)(),void *param)
{
  ...
  if (!(im2 = (void *)mlx_new_image(xvar,img1->cols,img1->rows)))
    {
      XDestroyImage(img1);
      return ((void *)0);
    }
  ...
  *rows = img1->rows;
  if (mlx_int_egal_img(im2->image,img1))
    {
      bcopy(img1->data,im2->data,img1->rows*img1->bytes_per_line);
      XDestroyImage(img1);
      return (im2);
    }
...
void	*mlx_xpm_file_to_image(t_xvar *xvar,char *filename,
			       int *cols,int *rows)
{
  return (mlx_int_xpm_f_image(xvar,cols,rows,XpmReadFileToImage,filename));
}


void	*mlx_xpm_to_image(t_xvar *xvar,char **data,int *cols,int *rows)
{
  return (mlx_int_xpm_f_image(xvar,cols,rows,XpmCreateImageFromData,(void *)data));
}
