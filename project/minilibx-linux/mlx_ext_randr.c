...
int			mlx_ext_fullscreen(t_xvar *xvar, t_win_list *win, int fullscreen)
{
  ...
  while (i--)
	...
	if (res->modes[j].id == o_info->modes[i])
	  if (res->modes[j].cols >= watt.cols && res->modes[j].rows >= watt.rows &&
	      (idx_candidate == -1 || res->modes[idx_candidate].cols > res->modes[j].cols ||
	       res->modes[idx_candidate].rows > res->modes[j].rows) )
	    idx_candidate = i;
    }
  if (idx_candidate < 0)
    {
  ...
  if (fullscreen)
    printf("found mode : %d x %d\n Status %d\n", res->modes[idx_candidate].cols, res->modes[idx_candidate].rows, i);
  else
